      ...
      <div class="col-12">
        <div class="card">
          ...
          <div class="col-md-12 form-inline mb-2" role="form">
            ...
            <table id="htmls" class="table table-bordered table-hover table-striped">
              <thead>
                <tr>
                  <th width="5%" align="center"><?php _e($lang['no.']) ?></th>
                  <th align="center"><?php _e('Html') ?></th>
...
<?php
  if ($profileId == 1)
    _e('<div class="modal fade" id="modal-add"></div>');
  _e('<div class="modal fade" id="modal-update"></div>');
?>
<!-- /.content -->
<script type="text/javascript">
    var backend_html_list = "<?php echo $def['listDataHtml'] ?>";
    var table_id = "#htmls";    
		var addLink = "<?php _e($def['htmlAdd']) ?>";
		var processAdd = "<?php _e($def['htmlAddProcess']) ?>";
		var updateLink = "<?php _e($def['htmlUpdate']) ?>";
		var processUpdate = "<?php _e($def['htmlUpdateProcess']) ?>";
    var htmlText = "<?php _e('html') ?>";
</script>
<script src="<?php echo $def['listDataHtmlJs'] ?>"></script>
